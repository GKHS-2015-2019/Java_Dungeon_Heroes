...
public class Inventory{
   public Map<Integer, Item> itemList;
   ...
   public int add(Item item){
      if(itemList.containsKey(item.uniqueID)){
         ...
         Item i = itemList.get(item.uniqueID);
         item.attack += 2;
         item.strength += 2;
         item.defense += 2;
         item.protection += 2;
         item.price += item.price;
         if(item.uniqueID >= 50 && item.uniqueID <= 100){
            item.speed -= 2;
         }
         System.out.println();
         ...
         System.out.println(item.name + " now has: " + item.protection + " Protection");
         System.out.println(item.name + " now has: " + item.speed + " Speed");
         return 0;
      } 
      else{
         //add the item
   ...
   public void list(){
      ...
      System.out.println("Item Num\tName\tType\tLevel");
      System.out.println("****************************************");
      for(Map.Entry<Integer, Item> entry : itemList.entrySet()){
         //print the item number and value
         System.out.print(entry.getKey() + "\t" + entry.getValue().name + entry.getValue().type + entry.getValue().level);
